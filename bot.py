...
Напиши /stats чтобы увидеть свою статистику"""
    ...
    try:
        ...
        print(f"👋 Вступление отправлено {user_name} (статус: {response.status_code})")
        return response.status_code == 200
    except Exception as e:
        print(f"❌ Ошибка отправки вступления: {e}")
        return False
...
Продолжай практиковать осознанность! 🧘"""
    ...
    try:
        response = requests.post(url, json=payload, timeout=10)
        print(f"📊 Статистика отправлена {user_name} (статус: {response.status_code})")
    except Exception as e:
        print(f"❌ Ошибка отправки статистики: {e}")

# 6. Основной цикл бота - ИСПРАВЛЕННАЯ ВЕРСИЯ
def bot_main_loop():
    ...
    # Ключевое: храним последний обработанный update_id
    last_update_id = 0
    
    # Сначала удаляем все старые обновления
    try:
        clear_url = f"https://api.telegram.org/bot{BOT_TOKEN}/getUpdates"
        requests.get(clear_url, params={"offset": -1}, timeout=5)
    ...
    while True:
        try:
            # 1. Получаем обновления с правильным offset
            ...
            if response.status_code == 200:
                ...
                if data.get('result'):
                    print(f"📨 Получено {len(data['result'])} обновлений")
                    ...
                    for update in data['result']:
                        ...
                        if 'message' in update and 'text' in update['message']:
                            ...
                            print(f"📩 Получено сообщение от {user.get('first_name')}: {text}")
                            ...
                            if text == '/start':
                                ...
                                if chat_id not in user_sessions:
                                    ...
                                    print(f"⚠️ Пользователь {user.get('first_name')} уже есть в сессии")
                                    # Отправляем повторное приветствие
                                    requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage", json={
                                        "chat_id": chat_id,
                                        "text": f"Привет снова, {user.get('first_name')}! Ты уже зарегистрирован. Следующий вопрос по расписанию.",
                                        "parse_mode": "Markdown"
                                    })
...
Просто нажимай на кнопки под сообщениями!"""
                        ...
                        # Обработка нажатий кнопок
                            ...
                            print(f"🖱️ Нажата кнопка от {user.get('first_name')}: {callback_data}")
            ...
            if active_users > 0 and int(time.time()) % 60 == 0:  # Каждую минуту
                print(f"👥 Активных пользователей: {active_users}")
                upcoming = len([t for t in question_schedule.values() if t - time.time() < 3600])
                if upcoming > 0:
                    print(f"⏰ Следующие вопросы: {upcoming} в течение часа")
...
if __name__ == "__main__":
    ...
    print(f"🔗 Веб-интерфейс доступен по: https://mindfulness-bot-1.onrender.com")
