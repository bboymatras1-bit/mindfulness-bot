...
print(f"🌐 Запускаю Flask сервер на порту {port} в фоновом режиме...")

# Запускаем веб-сервер в фоновом потоке (daemon=True)
web_thread = Thread(target=run_web_server, daemon=True)
web_thread.start()

print(f"✅ Flask сервер запущен. Основной поток свободен для запуска бота.")
print("-" * 50)

# ВАЖНО: Даем серверу пару секунд на инициализацию перед стартом бота
import time
time.sleep(2)

# ДАЛЕЕ ИДЕТ ТВОЙ ОРИГИНАЛЬНЫЙ КОД БОТА...
# (весь твой код с BOT_TOKEN, командами и app.run_polling())
# 
